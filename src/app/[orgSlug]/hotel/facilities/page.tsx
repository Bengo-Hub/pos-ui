...
import { Building2, CalendarPlus, Edit2, Loader2, Plus, Trash2, Users } from 'lucide-react';
...
const typeEmoji: Record<string, string> = {
  coworking: '💻', pool: '🏊', gym: '🏋️', conference: '🎙️', spa: '💆', kids_area: '🧒', other: '🏠',
};
...
// ─── Live seat-availability badge (shared/co-working facilities only) ─────────

function AvailabilityBadge({ facility }: { facility: Facility }) {
  const isShared = facility.booking_mode === 'shared';
  const { data, isLoading } = useFacilityAvailability(facility.id, todayStr(), isShared);
  if (!isShared) return null;
  if (isLoading || !data) return <span className="text-xs text-muted-foreground">Checking availability…</span>;
  const full = data.available_seats <= 0 && data.capacity > 0;
  return (
    <span className={cn('inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium', full ? 'bg-red-500/10 text-red-700 dark:text-red-400' : 'bg-green-500/10 text-green-700 dark:text-green-400')}>
      <Users className="h-3 w-3" /> {data.available_seats}/{data.capacity} seats free today
    </span>
  );
}
...
const emptyBooking = { guest_name: '', phone: '', session_date: todayStr(), start_time: '', end_time: '', guests_count: '1', seats: '1' };

function BookingForm({ facility, onDone }: { facility: Facility; onDone: () => void }) {
  const isShared = facility.booking_mode === 'shared';
  const [form, setForm] = useState(emptyBooking);
  const book = useBookFacility(facility.id);
  ...
  async function submit() {
    try {
      await book.mutateAsync({
        ...
        seats: isShared ? (parseInt(form.seats) || 1) : undefined,
      });
      toast.success(isShared ? 'Space assigned' : 'Facility booked');
      onDone();
    } catch (e) {
  ...
  const fields: { key: keyof typeof emptyBooking; label: string; placeholder?: string; type?: string }[] = [
    { key: 'guest_name', label: 'Customer Name', placeholder: 'Full name' },
    { key: 'phone', label: 'Phone', placeholder: '+254…' },
    { key: 'session_date', label: 'Date', type: 'date' },
    { key: 'start_time', label: 'Start (optional)', type: 'time' },
    { key: 'end_time', label: 'End (optional)', type: 'time' },
    isShared
      ? { key: 'seats', label: 'Seats', type: 'number', placeholder: '1' }
    ...
    <div className="space-y-3 border-t border-border pt-3">
      {isShared && (
        <p className="text-xs text-muted-foreground">
          Shared space — this assigns {form.seats || 1} seat(s) without blocking the whole space. Leave start/end blank for a full-day pass.
        </p>
      )}
      ...
      <div className="flex gap-2">
        ...
        <button onClick={submit} disabled={book.isPending} className="flex-1 rounded-lg bg-primary py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-50">
          {book.isPending ? 'Booking…' : isShared ? 'Assign Space' : 'Confirm'}
        </button>
      </div>
    </div>
  );
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {facilities.length} {facilities.length === 1 ? 'facility' : 'facilities'} — meeting rooms, co-working desks & more
          </p>
        </div>
        {canManage && (
          <button
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {facilities.map((facility) => {
            ...
            // bookable while status is available/occupied — the server enforces seat
            // capacity per request, so "occupied" (some seats taken) shouldn't hide the
            // Assign action the way it correctly does for a whole-room exclusive hold.
            const bookable = isShared
              ...
              <div key={facility.id} className="group flex flex-col rounded-2xl border border-border bg-card p-5 transition-all hover:border-primary/40 hover:shadow-md">
                  ...
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs capitalize text-muted-foreground">
                        {facility.facility_type.replace('_', ' ')} · {isShared ? 'Shared' : 'Exclusive'}
                      </p>
                    </div>
                  </div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
                  <div>
                    ...
                    <p className="font-medium text-foreground">{facility.capacity} {isShared ? 'seats' : 'guests'}</p>
                  </div>
                  <div>
                  ...
                  {isShared && (
                    <div className="col-span-2">
                      <AvailabilityBadge facility={facility} />
                    </div>
                  )}
                </div>

                <div className="mt-auto pt-4">
                  {bookable && bookingFacilityId !== facility.id && (
                    <button onClick={() => setBookingFacilityId(facility.id)} className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary/10 py-2 text-sm font-semibold text-primary hover:bg-primary/20">
                      <CalendarPlus className="h-4 w-4" /> {isShared ? 'Assign Space' : 'Book Session'}
                    </button>
                  )}
                  {bookingFacilityId === facility.id && (
                    <BookingForm facility={facility} onDone={() => setBookingFacilityId(null)} />
                  )}
                </div>
              </div>
            );
