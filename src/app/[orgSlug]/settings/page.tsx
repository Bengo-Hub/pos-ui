...
import { useStaffAdmin, useDeactivateStaff, useUpdateStaff, useSetStaffPIN } from '@/hooks/useStaff';
import type { StaffMember, UpdateStaffInput } from '@/lib/api/staff';

type Tab = 'general' | 'receipt' | 'modules' | 'shifts' | 'kds_stations' | 'tables' | 'integrations' | 'platform' | 'team';
...
// ══════════════════════════════════════════════════════════════════════════════
// Team tab — staff management (admin/manager only)
// ══════════════════════════════════════════════════════════════════════════════
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {members.map((m) => {
                    ...
                    const isProtected = (m.role === 'admin' || m.role === 'manager') && requesterRole === 'manager';
                    return (
                      <tr key={m.id} className="hover:bg-accent/5 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          {isEditing && !isProtected
                            ? (
                              <select className={inputClass} value={editForm.role ?? m.role} onChange={(e) => setEditForm((f) => ({ ...f, role: e.target.value }))}>
                        ...
                        <td className="px-4 py-3 text-center">
                          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${m.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>
                        ...
                        <td className="px-4 py-3 text-center">
                          {pinStaffId === m.id ? (
                            <div className="flex items-center gap-1 justify-center">
                              ...
                              <button onClick={() => { setPinStaffId(null); setNewPin(''); }} className="text-muted-foreground hover:text-foreground">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1 justify-end">
                            {isEditing ? (
                              <>
                                <Button size="sm" variant="ghost" className="h-7 px-2 text-xs" onClick={() => saveEdit(m)} disabled={update.isPending}>
                                  {update.isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : <Check className="h-3.5 w-3.5 text-green-600" />}
                                </Button>
